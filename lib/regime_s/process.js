exports.getCalcule = function(req,params, fn){
    ...
    req.getConnection(function(err,connection){
        ...
        var query = connection.query(' SELECT * FROM calcule  WHERE 1=1 '+if_search+'  ',[params.offset,params.limit],function(err,rows)  
        {
           var query = connection.query(' SELECT * FROM source  WHERE 1=1 ',function(err,rows1)  
        {
              var query = connection.query(' SELECT nom_boucle FROM calcule  group by nom_boucle',function(err,rows2)  
        {
                 var query = connection.query(' SELECT nom_boucle FROM inverse  group by nom_boucle',function(err,rows3)  
        {
         var query = connection.query(' SELECT * FROM inverse  WHERE 1=1 '+if_search+'  ',[params.offset,params.limit],function(err,rows4)  
        { 
            if(err)
                return fn(false,err);
            ...
            countAllCalcule(req,params,function(total){
                ...
                return fn(true,rows,rows1,rows2,rows3,rows4,total);
            });
             });
              }); 
              });
